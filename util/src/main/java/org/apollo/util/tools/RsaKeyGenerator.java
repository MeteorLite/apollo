...
public final class RsaKeyGenerator {
  ...
  /**
   * The bit count.
   * <strong>Note:</strong> 2048 bits and above are not compatible with the client without
   * modifications
   */
  private static final int BIT_COUNT = 1024;

  private static final String PRIVATE_KEY_DIRECTORY = "./data/";

  /**
   * The path to the private key file.
   */
  private static final String PRIVATE_KEY_FILE = "rsa.pem";
  ...
  public static void main(String[] args) throws Exception {
    ...
    new File(PRIVATE_KEY_DIRECTORY).mkdir();

    try (PemWriter writer = new PemWriter(
        new FileWriter(PRIVATE_KEY_DIRECTORY + PRIVATE_KEY_FILE))) {
