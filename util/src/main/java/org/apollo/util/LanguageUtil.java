...
public final class LanguageUtil {
  ...
  public static String getIndefiniteArticle(String string) {
    ...
    if (allUpperCase(string) && (first == 'f' || first == 'l' || first == 'm' || first == 'n'
        || first == 's')) {
      return "an";
    }
  ...
  /**
   * Returns whether or not the each letter in the specified String is upper case (i.e. digits etc
   * are ignored).
   *
   * @param string The string.
   * @return {@code true} if no letters in the specified String are lower case, otherwise
   * {@code false}.
   */
