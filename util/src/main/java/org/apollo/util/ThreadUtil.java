...
public final class ThreadUtil {
  ...
  /**
   * The default {@link UncaughtExceptionHandler} which raises an error from the logger with the
   * exception and name of the specified thread the exception occurred in.
   */
  private static final UncaughtExceptionHandler DEFAULT_EXCEPTION_HANDLER =
      (thread, exception) -> logger.log(Level.SEVERE, "Exception in thread " + thread.getName(),
          exception);
  ...
  /**
   * Builds a {@link ThreadFactory} using the specified {@code String} name-format, normal thread
   * priority and the default {@link UncaughtExceptionHandler}, which simply logs exception
   * messages.
   *
   ...
   * @param name     The name-format used when creating threads. Must not be {@code null}.
   * @param priority The priority used when creating threads. Must be {@code 1 <= priority <= 10}.
   * @param handler  The {@link UncaughtExceptionHandler} used when creating threads. Must not be
   *                 {@code null}.
   * @return The {@link ThreadFactory}. Will never be {@code null}.
   */
