...
public final class BufferUtil {
  ...
  public static int readSmart(ByteBuffer buffer) {
    ...
    int value = peek > Byte.MAX_VALUE ? (buffer.getShort() & 0xFFFF) + Short.MIN_VALUE
        : buffer.get() & 0xFF;
    return value;
  }
  ...
  public static String readString(ByteBuf buffer) {
    ...
    while (buffer.isReadable()
        && (character = buffer.readUnsignedByte()) != BufferUtil.STRING_TERMINATOR) {
  ...
  /**
   * Reads a 24-bit medium integer from the specified {@link ByteBuffer}s current position and
   * increases the buffers position by 3.
   *
