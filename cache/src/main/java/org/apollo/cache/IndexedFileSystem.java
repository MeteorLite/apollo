...
/**
 * A file system based on top of the operating system's file system. It consists of a data file and
 * index files. Index files point to blocks in the data file, which contains the actual data.
 *
 * @author Graham
 */
public final class IndexedFileSystem implements Closeable {
   ...
   */
  private final Map<FileDescriptor, Archive> cache = new HashMap<>(
      FileSystemConstants.ARCHIVE_COUNT);
  ...
  public ByteBuffer getFile(FileDescriptor descriptor) throws IOException {
    ...
    long position = (long) index.getBlock() * FileSystemConstants.BLOCK_SIZE;
    int read = 0;
   ...
   * @throws IOException               If there is an error getting the length of the specified
   *                                   index file.
   * @throws IndexOutOfBoundsException If {@code type} is less than 0, or greater than or equal to
   ...
   * @throws IndexOutOfBoundsException If the descriptor type is less than 0, or greater than or
   *                                   equal to the amount of indices.
   */
  private Index getIndex(FileDescriptor descriptor) throws IOException {
    ...
    synchronized (indexFile) {
      long position = (long) descriptor.getFile() * FileSystemConstants.INDEX_SIZE;
