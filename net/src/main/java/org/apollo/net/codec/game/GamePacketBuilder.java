...
public final class GamePacketBuilder {
  ...
  /**
   * Creates the {@link GamePacketBuilder} for a {@link PacketType#FIXED} packet with the specified
   * opcode.
   *
  ...
  public void putBits(int numBits, int value) {
    Preconditions.checkArgument(numBits >= 1 && numBits <= 32,
   ...
   * @throws IllegalStateException If the builder is not in byte access mode, or if the packet is
   *                               raw.
   */
  public GamePacket toGamePacket() {
    Preconditions.checkState(type != PacketType.RAW,
