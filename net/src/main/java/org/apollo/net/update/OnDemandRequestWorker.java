...
public final class OnDemandRequestWorker extends RequestWorker<OnDemandRequest, IndexedFileSystem> {
  ...
  @Override
  protected ChannelRequest<OnDemandRequest> nextRequest(UpdateDispatcher dispatcher)
  ...
  protected void service(IndexedFileSystem fs, Channel channel, OnDemandRequest request)
    ...
    for (int chunk = 0; buffer.readableBytes() > 0; chunk++) {
      ...
      OnDemandResponse response = new OnDemandResponse(descriptor, length, chunk,
