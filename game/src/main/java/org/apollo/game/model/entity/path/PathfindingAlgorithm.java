...
abstract class PathfindingAlgorithm {
  ...
  /**
   * Returns whether or not a {@link Position} walking one step in any of the specified
   * {@link Direction}s would lead to is traversable.
   *
   * @param current    The current Position.
   * @param directions The Directions that should be checked.
   * @return {@code true} if any of the Directions lead to a traversable tile, otherwise
   * {@code false}.
   */
   ...
   * {@code false}.
   */
  protected boolean traversable(Position current, Optional<Position[]> boundaries,
      Direction... directions) {
    Preconditions.checkArgument(directions != null && directions.length > 0,
        "Directions array cannot be null.");
    ...
    for (Direction direction : directions) {
      ...
      if (value >= Direction.NORTH_WEST.toInteger() && value <= Direction.NORTH_EAST.toInteger()) {
        y++;
      } else if (value >= Direction.SOUTH_WEST.toInteger()
          && value <= Direction.SOUTH_EAST.toInteger()) {
        y--;
      }

      if (direction == Direction.NORTH_EAST || direction == Direction.EAST
          || direction == Direction.SOUTH_EAST) {
        x++;
      } else if (direction == Direction.NORTH_WEST || direction == Direction.WEST
  ...
  /**
   * Returns whether or not the specified {@link Position} is inside the specified
   * {@code boundary}.
   *
