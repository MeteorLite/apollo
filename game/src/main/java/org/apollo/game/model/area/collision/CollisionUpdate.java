...
import static org.apollo.game.model.entity.obj.ObjectType.DIAGONAL_INTERACTABLE;
import static org.apollo.game.model.entity.obj.ObjectType.DIAGONAL_WALL;
import static org.apollo.game.model.entity.obj.ObjectType.FLOOR_DECORATION;
import static org.apollo.game.model.entity.obj.ObjectType.INTERACTABLE;
import static org.apollo.game.model.entity.obj.ObjectType.LENGTHWISE_WALL;
...
import static org.apollo.game.model.entity.obj.ObjectType.WALL_CORNER;
...
public final class CollisionUpdate {
  ...
  /**
   * Returns whether or not an object with the specified {@link ObjectDefinition} and {@code type}
   * should result in the tile(s) it is located on being blocked.
   *
  ...
  private static boolean unwalkable(ObjectDefinition definition, int type) {
    boolean isSolidFloorDecoration =
        type == FLOOR_DECORATION.getValue() && definition.isInteractive();
  ...
  /**
   * A directional flag in a {@code CollisionUpdate}. Consists of a {@code direction} and a flag
   * indicating whether that tile is impenetrable as well as untraversable.
   */
  public static final class DirectionFlag {
     ...
     *
     * @return {@code true} if this flag represents an impenetrable direction, {@code false}
     * otherwise.
     */
  ...
  public static final class Builder {
    ...
    public void tile(Position position, boolean impenetrable, Direction... directions) {
      ...
      Stream.of(directions)
          .forEach(direction -> flags.put(position, new DirectionFlag(impenetrable, direction)));
    }

    /**
     * Flag a wall in the CollisionUpdate.  When constructing a CollisionMatrix, the flags for a
     * wall are represented as the tile the wall exists on and the tile one step in the facing
     * direction. So for a wall facing south, the tile one step to the south be flagged as
    ...
    /**
     * Flag a larger corner wall in the CollisionUpdate.  A corner is represented by the 2
     * directions that it faces, and the 2 tiles in both directions.  For example, when a tile is
     * facing north its facing directions are north and east, so the position of the object will be
     * untraversable from those directions.  Additionally, the tile 1 step to the north, and 1 step
     * to the east will be untraversable from the opposite directions of north and east
     * respectively.
     * <p>
