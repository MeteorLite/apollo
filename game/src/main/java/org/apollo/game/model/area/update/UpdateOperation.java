...
/**
 * An type that is contained in the snapshot of a {@link Region}, which consists of an
 * {@link Entity} being added, removed, or moved.
 *
...
public abstract class UpdateOperation<E extends Entity> {
   ...
   *
   * @param offset The offset of the {@link Position} of the Entity from the Position of the
   *               {@link Region}.
   * @return The RegionUpdateMessage.
   */
  ...
  private final int getPositionOffset(Position position) {
    ...
    Preconditions.checkArgument(dx >= 0 && dx < Region.SIZE,
        position + " not in expected Region of " + region + ".");
    Preconditions.checkArgument(dy >= 0 && dy < Region.SIZE,
        position + " not in expected Region of " + region + ".");
