...
public final class CollisionManager {
  ...
  /**
   * A {@code HashMultimap} of region coordinates mapped to positions where the tile is completely
   * blocked.
   */
  ...
  /**
   * Applies the initial {@link CollisionUpdate} to the {@link CollisionMatrix}es for all objects
   * and tiles loaded from the cache.
   *
   * @param rebuilding A flag indicating whether or not {@link CollisionMatrix}es are being
   *                   rebuilt.
   */
  ...
  /**
   * Casts a ray into the world to check for impenetrable objects  from the given {@code start}
   * position to the {@code end} position using Bresenham's line algorithm.
   *
  ...
  public boolean raycast(Position start, Position end) {
    Preconditions.checkArgument(start.getHeight() == end.getHeight(),
        "Positions must be on the same height");
   ...
   */
  private void flag(CollisionUpdateType type, CollisionMatrix matrix, int localX, int localY,
      CollisionFlag flag) {
  ...
  /**
   * Checks if the given {@link EntityType} can traverse to the next tile from {@code position} in
