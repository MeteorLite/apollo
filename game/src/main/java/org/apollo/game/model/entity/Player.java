...
public final class Player extends Mob {
   ...
   * This player's bank.
   */
  private final Inventory bank = new Inventory(InventoryConstants.BANK_CAPACITY,
   ...
   * @return {@code true} if the specified username is on this player's friend list, otherwise
   * {@code false}.
   */
  ...
  /**
   * Indicates whether or not the player with the specified username is on this player's ignore
   * list.
   *
  ...
  public void openBank() {
    InventoryListener invListener = new SynchronizationInventoryListener(this,
    ...
    interfaceSet.openWindowWithSidebar(interListener, BankConstants.BANK_WINDOW_ID,
   ...
   *
   * @param username The username.
   * @return {@code true} if the player's friend list contained the specified user, {@code false} if
   * not.
   */
   ...
   *
   * @param username The username.
   * @return {@code true} if the player's ignore list contained the specified user, {@code false} if
   * not.
   */
  ...
  /**
   * Removes the specified {@link DynamicGameObject} from this Player's {@link Set} of visible
   * objects.
   *
   * @param object The DynamicGameObject.
   */
  ...
  public void sendQuestInterface(List<String> text) {
    ...
    Preconditions.checkArgument(size <= lines,
        "List contains too much text to display on this interface.");

    for (int pos = 0; pos < lines; pos++) {
      send(new SetWidgetTextMessage(InterfaceConstants.QUEST_TEXT[pos],
          pos < size ? text.get(pos) : ""));
    }
  ...
  public String toString() {
    return MoreObjects.toStringHelper(this).add("username", getUsername())
        .add("privilege", privilegeLevel)
        .toString();
  }
  ...
  private void initInventories() {
    InventoryListener fullInventory = new FullInventoryListener(this,
        FullInventoryListener.FULL_INVENTORY_MESSAGE);
    InventoryListener fullBank = new FullInventoryListener(this,
    ...
    InventoryListener syncBank = new SynchronizationInventoryListener(this,
        BankConstants.BANK_INVENTORY_ID);
