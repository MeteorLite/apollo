...
public abstract class Mob extends Entity {
  ...
  public final void damage(int damage, int type, boolean secondary) {
    ...
    int current = Math.max(hitpoints.getCurrentLevel() - damage,
        0), maximum = hitpoints.getMaximumLevel();

    blockSet.add(
        SynchronizationBlock.createHitUpdateBlock(damage, type, current, maximum, secondary));
   ...
   * <p>
   * This method may be intercepted using a {@link MobPositionUpdateEvent}, which can be terminated
   * like any other. Plugins that intercept this Event <strong>must</strong> be cautious, because
   * movement will not be possible (even through mechanisms such as teleporting) if the Event is
   * terminated.
   *
   * @param position The Position.
   */
  public final void setPosition(Position position) {
    if (!position.equals(this.position) && world.submit(
        new MobPositionUpdateEvent(this, position))) {
  ...
  /**
   * Forces this mob to shout a message. Only messages said by a player can be shown in the chat
   * box.
   *
   * @param message  The message.
   * @param chatOnly If the message should only be shown in the player's chat box, or above their
   *                 head too.
   */
  ...
  /**
   * Teleports this mob to the specified {@link Position}, setting the appropriate flags and
   * clearing the walking queue.
   *
   * @param position The position.
   */
