...
public final class Position {
   ...
   * @return A new {@code Position} that is {@code num} steps in {@code direction} ahead of this
   * one.
   */
  public Position step(int num, Direction direction) {
    return new Position(getX() + (num * direction.deltaX()), getY() + (num * direction.deltaY()),
        getHeight());
  }

  @Override
  public String toString() {
    return MoreObjects.toStringHelper(this).add("x", getX()).add("y", getY())
        .add("height", getHeight()).add("map", getRegionCoordinates()).toString();
  }
}