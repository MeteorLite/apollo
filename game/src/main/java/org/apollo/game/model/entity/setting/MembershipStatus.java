...
public enum MembershipStatus {
  ...
  public static MembershipStatus valueOf(int value) {
    Optional<MembershipStatus> optional = Arrays.stream(values())
        .filter(status -> status.value == value).findAny();
    return optional.orElseThrow(
        () -> new IllegalArgumentException("Illegal membership status value."));
  }
