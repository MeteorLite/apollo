...
public final class Inventory {
  ...
  /**
   * Attempts to add as much of the specified {@code item} to this inventory as possible. If any of
   * the item remains, an {@link Item item with the remainder} will be returned (in the case of
   * stack-able items, any quantity that remains in the stack is returned). If nothing remains, the
   * method will return {@link Optional#empty an empty Optional}.
   *
   * <p>
   * If anything remains at all, the listener will be notified which could be used for notifying a
   * player that their inventory is full, for example.
   *
   * @param item The item to add to this inventory.
   * @return The item that may remain, if nothing remains, {@link Optional#empty an empty Optional}
   * is returned.
   */
   ...
   *
   * @param ids The ids.
   * @return {@code true} if the inventory does contain at least one of the items, otherwise
   * {@code false}.
   */
   ...
   * <p>
   * This method will attempt to remove one of each item, and will continue even if a previous item
  ...
  /**
   * Removes {@code amount} of the item with the specified {@code id}. If the item is stackable, it
   * will remove it from the stack. If not, it'll remove {@code amount} items.
   *
   * @param id     The id.
   * @param amount The amount.
  ...
  /**
   * Remove all items with the given {@code id} and return the number of items removed.
   *
   * @param id The id of items to remove.
   * @return The amount that was removed.
   */
  public int removeAll(int id) {
    return remove(id, getAmount(id));
  }
  ...
  /**
   * Removes {@code amount} of the item at the specified {@code slot}. If the item is not stacked,
   * it will only remove the single item at the slot (meaning it will ignore any amount higher than
   * 1). This means that this method will under no circumstances make any changes to other slots.
   *
   * @param slot   The slot.
   ...
   *
   * @param id The id.
   * @return The first slot containing the specified item, or {@code -1} if none of the slots
  ...
  public static enum StackMode {
    ...
    /**
     * When in {@link #STACK_ALWAYS} mode, an {@link Inventory} will stack every single item,
     * regardless of the settings of individual items.
     */
    STACK_ALWAYS,
    ...
    /**
     * When in {@link #STACK_STACKABLE_ITEMS} mode, an {@link Inventory} will stack items depending
     * on their settings.
     */
