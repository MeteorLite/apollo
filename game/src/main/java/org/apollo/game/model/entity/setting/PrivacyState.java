...
/**
 * An enumeration representing the different privacy states for public, private and trade chat. This
 * enumeration relies on the ordering of the elements within, which should be as follows:
 * {@code ON}, {@code HIDE}, {@code FRIENDS}, {@code OFF}, {@code FILTERABLE}.
 *
...
public enum PrivacyState {
  ...
  /**
   * Represents the 'hidden' state, when all public chat text is displayed over the heads of
   * players, but not in the chat interface. This state only applies to public chat.
   */
  HIDE(1),
   ...
   * This state only applies to public chat.
   */
  FILTERABLE(4);
  ...
  public static PrivacyState valueOf(int value, boolean chat) {
    ...
    Preconditions.checkElementIndex(value, values.length,
        "Invalid privacy option integer value specified: " + value + ".");
    return values[value];
  }
