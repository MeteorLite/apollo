...
public final class CollisionMatrix {
  ...
  /**
   * Returns whether or not <strong>all</strong> of the specified {@link CollisionFlag}s are set for
  ...
  /**
   * Returns whether or not <strong>any</strong> of the specified {@link CollisionFlag}s are set for
   * the specified coordinate pair.
   *
   * @param x     The x coordinate.
   * @param y     The y coordinate.
  ...
  /**
   * Completely blocks the tile at the specified coordinate pair, while optionally allowing
  ...
  /**
   * Clears (i.e. sets to {@code false}) the value of the specified {@link CollisionFlag} for the
  ...
  /**
   * Returns whether or not the specified {@link CollisionFlag} is set for the specified coordinate
   * pair.
   *
   * @param x    The x coordinate.
   * @param y    The y coordinate.
  ...
  /**
   * Sets (i.e. sets to {@code true}) the value of the specified {@link CollisionFlag} for the
   * specified coordinate pair.
   *
   * @param x    The x coordinate.
   * @param y    The y coordinate.
  ...
  /**
   * Returns whether or not an Entity of the specified {@link EntityType type} cannot traverse the
   * tile at the specified coordinate pair.
   *
   * @param x         The x coordinate.
   * @param y         The y coordinate.
  ...
  public boolean untraversable(int x, int y, EntityType entity, Direction direction) {
    ...
    switch (direction) {
      case NORTH_WEST:
        return flagged(x, y, flags[southeast]) || flagged(x, y, flags[south]) || flagged(x, y,
            flags[east]);
      case NORTH:
        return flagged(x, y, flags[south]);
      case NORTH_EAST:
        return flagged(x, y, flags[southwest]) || flagged(x, y, flags[south]) || flagged(x, y,
            flags[west]);
      case EAST:
        return flagged(x, y, flags[west]);
      case SOUTH_EAST:
        return flagged(x, y, flags[northwest]) || flagged(x, y, flags[north]) || flagged(x, y,
            flags[west]);
      case SOUTH:
        return flagged(x, y, flags[north]);
      case SOUTH_WEST:
        return flagged(x, y, flags[northeast]) || flagged(x, y, flags[north]) || flagged(x, y,
            flags[east]);
      case WEST:
   ...
   * @param y The y coordinate.
   * @return The index.
   * @throws ArrayIndexOutOfBoundsException If the specified coordinate pair does not fit in this
   *                                        matrix.
   */
  private int indexOf(int x, int y) {
    Preconditions.checkElementIndex(x, width,
        "X coordinate must be [0, " + width + "), received " + x + ".");
    Preconditions.checkElementIndex(y, length,
        "Y coordinate must be [0, " + length + "), received " + y + ".");
    return y * width + x;
  }
