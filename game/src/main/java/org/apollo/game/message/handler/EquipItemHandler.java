...
public final class EquipItemHandler extends MessageHandler<ItemOptionMessage> {
  ...
  public void handle(Player player, ItemOptionMessage message) {
    ...
    if (equipmentSlot == SHIELD && weapon != null && EquipmentDefinition.lookup(weapon.getId())
        .isTwoHanded()) {
    ...
    if (current != null && current.getId() == equippingId && current.getDefinition()
        .isStackable()) {
   ...
   */
  private void handleTwoHanded(Inventory inventory, Inventory equipment, int inventorySlot,
      Item weapon, Item shield, Player player, ItemOptionMessage message) {
  ...
  /**
   * Returns whether or not the specified {@link Player} has the required levels to equip the item
   * with the specified {@link EquipmentDefinition}, sending the {@link Player} a message if not.
   *
   * @param player     The {@link Player} trying to equip the item. Must not be {@code null}.
   * @param definition The {@link EquipmentDefinition} of the item being equipped. Must not be
   *                   {@code null}.
  ...
  private boolean hasRequirements(Player player, EquipmentDefinition definition) {
    ...
    for (int id = Skill.ATTACK; id <= Skill.MAGIC; id++) {
      ...
      if (skills.getMaximumLevel(id) < requirement) {
        ...
        player.sendMessage(
            "You need to have " + LanguageUtil.getIndefiniteArticle(name) + " " + name.toLowerCase()
