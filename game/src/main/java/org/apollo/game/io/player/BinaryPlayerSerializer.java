...
public final class BinaryPlayerSerializer extends PlayerSerializer {
  ...
  public PlayerLoaderResponse loadPlayer(PlayerCredentials credentials) throws IOException {
    ...
    try (DataInputStream in = new DataInputStream(
      ...
      if (!name.equalsIgnoreCase(credentials.getUsername()) || !SCryptUtil.check(
      ...
      Gender gender =
          in.readUnsignedByte() == Gender.MALE.toInteger() ? Gender.MALE : Gender.FEMALE;
      int[] style = new int[7];
      ...
      try {
        for (int i = 0; i < size; i++) {
          ...
          skills.setSkill(i,
              new Skill(experience, level, SkillSet.getLevelForExperience(experience)));
        }
      } finally {
  ...
  public void savePlayer(Player player) throws IOException {
    ...
    try (DataOutputStream out = new DataOutputStream(Files.newOutputStream(file))) {
      ...
      attributes.removeIf(e -> AttributeMap.getDefinition(e.getKey()).getPersistence()
          != AttributePersistence.PERSISTENT);
