...
/**
 * A {@link SynchronizationTask} which does pre-synchronization work for the specified
 * {@link Player}.
 *
 * @author Graham
 * @author Major
...
public final class PrePlayerSynchronizationTask extends SynchronizationTask {
  ...
  /**
   * The Map of RegionCoordinates to Sets of RegionUpdateMessages, which contain all of the Entity
  ...
  /**
   * The Map of RegionCoordinates to Sets of RegionUpdateMessages, which contain the updates for a
   * Region a Player can already view.
   */
  ...
  public PrePlayerSynchronizationTask(Player player,
      Map<RegionCoordinates, Set<RegionUpdateMessage>> encodes,
   ...
   * @param full        The {@link Set} of {@link RegionCoordinates} of Regions that require a full
   *                    update.
   */
  private void sendUpdates(Position position, Set<RegionCoordinates> differences,
      Set<RegionCoordinates> full) {
    RegionRepository repository = player.getWorld().getRegionRepository();
