...
public final class PlayerSynchronizationTask extends SynchronizationTask {
  ...
  /**
   * The maximum number of players to load per cycle. This prevents the update packet from becoming
   * too large (the client uses a 5000 byte buffer) and also stops old spec PCs from crashing when
   * they login or teleport.
   */
  ...
  public void run() {
    ...
    SynchronizationSegment segment = (player.isTeleporting() || player.hasRegionChanged()) ?
        new TeleportSegment(blockSet, position)
        : new MovementSegment(blockSet, player.getDirections());
    ...
    while (iterator.hasNext()) {
      ...
      if (other != player && local.isWithinDistance(position, distance) && !localPlayers.contains(
          other)) {
        ...
        if (!blockSet.contains(AppearanceBlock.class) && !hasCachedAppearance(appearanceTickets,
            index - 1, other.getAppearanceTicket())) {
          blockSet = blockSet.clone();
    ...
    PlayerSynchronizationMessage message = new PlayerSynchronizationMessage(lastKnownRegion,
        position,
