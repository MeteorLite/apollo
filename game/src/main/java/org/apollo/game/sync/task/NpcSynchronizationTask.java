...
public final class NpcSynchronizationTask extends SynchronizationTask {
  ...
  /**
   * The maximum number of npcs to load per cycle. This prevents the update packet from becoming too
   * large (the client uses a 5000 byte buffer) and also stops old spec PCs from crashing when they
   * login or teleport.
   */
  ...
  public void run() {
    ...
    for (Iterator<Npc> iterator = locals.iterator(); iterator.hasNext(); ) {
      ...
      if (!npc.isActive() || npc.isTeleporting()
          || position.getLongestDelta(playerPosition) > distance
