...
/**
 * A synchronization block is part of a {@link SynchronizationSegment}. A segment can have up to one
 * block of each type.
 * <p>
...
public abstract class SynchronizationBlock {
  ...
  public static SynchronizationBlock createAppearanceBlock(Player player) {
    ...
    return new AppearanceBlock(player.getEncodedName(), player.getAppearance(), combat, 0,
   ...
   * @param travelDurationX The length of time (in game pulses) the player's movement along the X
   *                        axis will last.
   * @param travelDurationY The length of time (in game pulses) the player's movement along the Y
   *                        axis will last.
  ...
  public static SynchronizationBlock createForceMovementBlock(Position initialPosition,
    ...
    return new ForceMovementBlock(initialPosition, finalPosition, travelDurationX, travelDurationY,
        direction);
  }
  ...
  /**
   * Creates a {@link HitUpdateBlock} or {@link SecondaryHitUpdateBlock}, depending on the value of
   * the {@code secondary} flag.
   *
   ...
   */
  public static SynchronizationBlock createHitUpdateBlock(int damage, int type, int currentHealth,
      int maximumHealth, boolean secondary) {
