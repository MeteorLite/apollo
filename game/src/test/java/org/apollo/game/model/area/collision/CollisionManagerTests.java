...
public final class CollisionManagerTests {
  ...
  /**
   * Helper function for creating {@code org.apollo.cache} {@link MapObject}s using data structures
   * from {@code org.apollo.game}.
   *
   * @param id        The id of the object.  Must be one of the {@link ObjectDefinition}s defined
   *                  above.
   ...
   */
  private static MapObject createMapObject(int id, Position position, ObjectType type,
      Direction direction) {
    return new MapObject(id, position.getX(), position.getY(), position.getHeight(),
        type.getValue(),
        direction.toOrientationInteger());
  }

  /**
   * Sets up dependencies for and creates a stub {@link CollisionManager}, then builds the collision
   * matrices using the {@code objects} given.
   *
   * @param objects The objects to build collision matrices from.
   * @return A new {@link CollisionManager} with a valid {@link RegionRepository} and every
   * {@link CollisionMatrix} built.
   */
   ...
   */
  private static void assertUntraversable(CollisionManager collisionManager, Position position,
   ...
   * @param directions       The {@link Direction}s to assert.
   */
  private static void assertTraversable(CollisionManager collisionManager, Position position,
      Direction... directions) {
    for (Direction direction : directions) {
      boolean traversable = collisionManager.traversable(position, EntityType.NPC, direction);
  ...
  /**
   * Tests that a wall is untraversable from the front and back, for the facing direction and
   * opposite facing direction respectively. For a simple grid, with a wall on 0,1 we end up with a
   * collision matrix looking like:
   * <pre>
   * (0,2) |---------|
   *       |         |
   *       |         |
   ...
   * </pre>
   * <p>
   * Where {@code xxx} denotes that you cannot walk into that tile from that direction (in this case
   * you south on (0,2) and north on (0,1)). For the grid above you can't walk north into
   * {@code (0, 2)} because {@code (0, 2)} is untraversable from the south, and as expectred you
   * can't walk south into {@code (0,1)} because ({@code (0, 1)} is untraversable from the north.
   */
  @Test
  public void wall() {
  ...
  /**
   * Tests that a corner triangular wall is untraversable from the sides that it blocks.  Corners
   * are much like walls, with the only difference being their orientation.  Instead of
   * {@link Direction#WNES}, they have diagonal directions. With a corner wall at (0, 1) facing to
   * the north-east we end up with a grid looking like this:
   * <p>
   * <pre>
   * (0,2) |---------|---------|
   *       |         |         |
   ...
   * </pre>
   * <p>
   * Where you can walk north and east through the tile the corner occupies, as well as south and
   * west through the adjacent tile, but not north-east or south-west.
   */
  @Test
  ...
  /**
   * Tests that a corner rectangle wall is untraversable from the sides that it blocks.  Corners are
   * much like walls, with the only difference being their orientation.  Instead of
   * {@link Direction#WNES}, they have diagonal directions. With a corner wall at (0, 1) facing to
   * the north-east we end up with a grid looking like this:
   * <p>
   * <pre>
  ...
  /**
   * Tests that the tiles occupied by a 2x2 square object are not traversable.  When an interactable
   * object is added to a collision update, all tiles spanning its with and length from its origin
   * position will be marked as completely blocked off, which with a 2x2 object at (1,1) produces a
   * grid like this:
   * <pre>
   * (0,3) |---------|---------|---------|---------|
   *       |         |xxxxxxxxx|xxxxxxxxx|         |
