    ...
    companion object {
        ...
        fun start(message: ObjectActionMessage, player: Player, wood: Tree) {
            ...
            if (axe != null) {
                ...
                val action = WoodcuttingAction(
                        player,
                        axe,
                        WoodcuttingTarget(message.id, message.position, wood)
                )
    ...
    override fun action(): ActionBlock = {
        ...
        while (isRunning) {
            ...
            if (mob.inventory.add(target.tree.id)) {
                if (mob.tutorialProgress == 2) {
                    mob.lastDialogue = Dialogues.SURVIVAL_EXPERT_3
                    Dialogue.itemOne(mob,
                        "",
                        "You got some logs!",
                        Items.LOGS.id, 150)
                } else {
            ...
            if (target.isCutDown()) {
                ...
                val respawn =
                        TimeUnit.SECONDS.toMillis(MINIMUM_RESPAWN_TIME + rand(150)) / GameConstants.PULSE_DELAY
