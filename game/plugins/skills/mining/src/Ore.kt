...
enum class Ore(
    ...
    companion object {
        private val ORE_ROCKS =
                Ore.values().flatMap { ore -> ore.objects.map { Pair(it.key, ore) } }.toMap()
        private val EXPIRED_ORE =
                Ore.values().flatMap { ore -> ore.objects.map { Pair(it.value, ore) } }.toMap()
