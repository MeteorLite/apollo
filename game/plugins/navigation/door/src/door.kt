...
class Door(private val gameObject: GameObject) {
    ...
    fun toggle() {
        ...
        if (originalDoor == null) {
            ...
            val orientation: Int =
                    translateDirection()?.toOrientationInteger() ?: gameObject.orientation

            val toggledDoor = DynamicGameObject.createPublic(
                    world, gameObject.id, position, gameObject.type,
                    orientation
            )
...
class OpenDoorAction(private val player: Player, private val door: Door, position: Position) :
